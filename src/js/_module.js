...
var Module = {
  ...
  imageUploader: null,
  controls:
    '<div class="module-controls-container" style="top: 10px"><!-- Controls -->' +
    ...
    "<!-- /Controls --></div>",
  codeControls:
    '<div class="module-code-controls">' +
  ...
  init: function () {
    ...
    $(".module-container", '[data-type="editor"]').on(
      "mouseenter",
      function (event) {
  ...
  controlsDuplicate: function () {
    ...
    $(".btn-module-duplicate", ".module-controls-container").on(
      "click",
      function (event) {
        ...
        module
          .clone()
          .insertAfter(module)
          .find(".module-controls-container")
          .remove();
  ...
  controlsCode: function () {
    ...
    $(".btn-module-code", ".module-controls-container").on(
      "click",
      function (event) {
        var module = $(this).closest(".module-container");
        ...
        moduleTmpHTML = moduleTmpHTML
          .replace(/<tbody>/g, "")
          .replace(/<\/tbody>/g, "");

        moduleTmpHTML = moduleTmpHTML.replace(
          /(v:)+([\w])|(xmlns)+(:v)/gi,
          function vml_pre_replace(x) {
  ...
  controlsDelete: function () {
    ...
    $(".btn-module-delete", ".module-controls-container").on(
      "click",
      function (event) {
        var module = $(this).closest(".module-container");
  ...
  optionActions: function () {
    ...
    $('[data-control="colorpicker"]').on("change", function (event) {
      ...
      $("[data-" + targetAttr + '="' + targetOption + '"]', self.current).css(
        targetAttr,
        currentVal
      );
      $("[data-" + targetAttr + '="' + targetOption + '"]', self.current).attr(
        targetAttr,
        currentVal
      );
      emitter.emit("update_template");
    ...
    $('[data-control="background-image"]').on("click", function (event) {
      ...
      var $target = $(event.target), // clicked button
        ...
        targetImageSrc = $(this).data("bg-image"), // Current Image Src
      ...
      $.fancybox.open({
        ...
        opts: {
          ...
          afterShow: function (instance, current) {
            ...
            $(".modal-btn-cancel", modalContainer).on(
              "click",
              function (event) {
                // Close Modal
                instance.close();
            ...
            $(".modal-btn-ok", modalContainer).on("click", function (event) {
              ...
              var imageSrc = $('[data-image="src"]', modalContainer).attr(
                "src"
              );

              // Update background label
              $(
                '[data-bg-option-label="' + option_id + '"]',
                $target.parent()
              ).html(imageSrc);
              // Update module background
              $('[data-bg="' + option_id + '"]', Module.current).css(
                "background-image",
                "url('" + imageSrc + "')"
              );
    ...
    $('[data-control="background-appearance"]').on("change", function (event) {
      ...
      $('[data-bg="' + targetOption + '"]', self.current).css(
        "background-position",
        ""
      );
      $('[data-bg="' + targetOption + '"]', self.current).css(
  ...
  getCodeEditorHTML: function (moduleHTML = "") {
    var codeEditorHTML =
      '<div class="module-code-container">' +
      this.codeControls +
  ...
  getOptions: function () {
    ...
    self.options["color"] = colorOptions.filter(
      (v, i, a) => a.indexOf(v) === i
    );
    self.options["bgcolor"] = bgcolorOptions.filter(
      (v, i, a) => a.indexOf(v) === i
    );
  ...
  getOptionsHTML: function () {
    ...
    if (self.options["color"].length > 0) {
      ...
      self.options["color"].forEach(function (option) {
        // get option value
        var color = $('[data-color="' + option + '"]', self.current).css(
          "color"
        );
        var val = self.rgb2hex(color);
        optionsHTML += '        <div class="style-item">';
        optionsHTML +=
          '            <div class="style-label">' + option + "</div>";
        optionsHTML +=
          '            <div class="style-control"><input type="text" value="' +
          val +
          '" data-control="colorpicker" data-target-option="' +
          option +
          '" data-target-attr="color"></div>';
        optionsHTML += "        </div>";
      });
      optionsHTML += "    </div>";
      optionsHTML += "</div>";
    ...
    if (self.options["bgcolor"].length > 0) {
      ...
      self.options["bgcolor"].forEach(function (option) {
        // get option value
        var color = $('[data-bgcolor="' + option + '"]', self.current).css(
          "background-color"
        );
        var val = self.rgb2hex(color);
        optionsHTML += '        <div class="style-item">';
    ...
    if (self.options["bg"].length > 0) {
      ...
      self.options["bg"].forEach(function (option) {
        var bgImage = $('[data-bg="' + option + '"]', self.current)
        ...
        optionsHTML +=
          '            <div class="style-label">' + option + "</div>";
        optionsHTML +=
          '            <div class="style-control"><span data-bg-option-label="' +
          option +
          '">' +
          bgImage +
          '</span><button data-control="background-image" data-bg-image="' +
          bgImage +
          '" data-target-option="' +
          option +
          '">Change</button></div>';
        ...
        optionsHTML +=
          '                <select data-control="background-appearance" data-target-option="' +
          option +
          '">';
        optionsHTML +=
